...
class ProductOrderController extends Controller
{
    ...
    public function status(Request $request)
    {
        ...
        $sub = 'Order Status Update';
         ...
         if ($em->is_smtp == 1) {
             try {
                ...
                $mail->Port       = $em->smtp_port;
                 ...
                 $mail->isHTML(true);
                 $mail->Subject = $sub;
                 $mail->Body    = 'Hello <strong>' . $user->name . '</strong>,<br/>O status do seu pedido de produto é '.$ostatus.'.<br/>Obrigado.';
                 $mail->send();
             } catch (Exception $e) {
                 // die($e->getMessage());
             }
             ...
             try {
                 ...
                 $mail->isHTML(true);
                 $mail->Subject = $sub ;
                 $mail->Body    = 'Olá <strong>' . $user->name . '</strong>,<br/>O status do seu pedido de produto é '.$ostatus.'.<br/>Obrigado.';
         ...
         $notification = array(
            'messege' => 'Status do pedido alterado com sucesso!',
            'alert' => 'success'
        );
         ...
         if ($em->is_smtp == 1) {
             try {
                ...
                $mail->Port       = $em->smtp_port;
                 ...
                 $mail->isHTML(true);
                 $mail->Subject = $sub;
                 $mail->Body    = 'Olá <strong>' . $user->name . '</strong>,<br/>O status do seu pedido é '.$ostatus.'.<br/>Obrigado.';
                 $mail->send();
             } catch (Exception $e) {
                 // die($e->getMessage());
             }
    ...
    public function orderDelete(Request $request)
    {
        ...
        $notification = array(
            'messege' => 'pedido de produto deletado com sucesso!',
            'alert' => 'success'
        );
        return redirect()->back()->with('notification', $notification);
    }
