        ...
        foreach ($menus as $key => $menu) {
            if (strpos($menu['type'], 'products') !== false) {
                if (array_key_exists('children', $menu) && !empty($menu['children'])) {
                    return response()->json(['status' => 'error', 'message' => 'Menu de produtos não pode conter filhos!']);
                }
            }
            if (array_key_exists('children', $menu) && !empty($menu['children'])) {
                $allChildren = json_encode($menu['children']);
                if (strpos($allChildren, 'products') !== false) {
                    return response()->json(['status' => 'error', 'message' => 'O Menu de Produto não pode ser filho de um Menu!']);
                }
            }
        }
