                ...
                <div class="col-md-12">
                    <div class="card card-primary card-outline">
                        <div class="card-header">
                            <h3 class="card-title mt-1">{{ __('Lista de Backup') }}</h3>
                            <div class="card-tools d-flex">
                                <form  action="{{route('admin.backup.store')}}" method="post">
                                    @csrf
                                    <button type="submit" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i> Criar Backup </button>
                                </form>
                            </div>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            @if (count($backups) == 0)
                                <h3 class="text-center">{{ __('NENHUM BACKUP ENCONTRADO') }}</h3>
                            @else
