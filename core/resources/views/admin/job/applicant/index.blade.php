...
<div class="content-header">
    <div class="container-fluid">
        <div class="row">
        <div class="col-sm-6">
            <h1 class="m-0 text-dark">{{ __('Candidatos') }}</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"><i class="fas fa-home"></i>{{ __('Home') }}</a></li>
            <li class="breadcrumb-item">{{ __('Candidatos') }}</li>
            </ol>
        </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card card-primary card-outline">
                    ...
                    <h3 class="card-title">{{ __('Lista de Candidatos:') }}</h3>
                    <div class="card-tools">
                      <form class="d-inline-block mr-3" action="{{route('back.bulk.delete')}}" method="get">
                        ...
                        <button class="btn btn-danger btn-sm"><i class="far fa-trash-alt"></i> {{__('Excluir Todos')}}</button>
                    </form>
                    ...
                    <table class="table table-striped table-bordered data_table">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('Cargo') }}</th>
                                <th>{{ __('Posição de Trabalho') }}</th>
                                <th>{{ __('Data de candidatura') }}</th>
                                <th>{{ __('Status') }}</th>
                                <th>{{ __('Ação') }}</th>
                            </tr>
                        </thead>
                                ...
                                <td>
                                    <form id="statusForm{{$applicant->id}}" class="d-inline-block" action="{{route('admin.application.status')}}" method="post">
                                        ...
                                        " name="status" onchange="document.getElementById('statusForm{{$applicant->id}}').submit();">
                                          <option value="0" {{$applicant->status == '0' ? 'selected' : ''}}>Pendente</option>
                                          <option value="1" {{$applicant->status == '1' ? 'selected' : ''}}>Intrevistando</option>
                                          <option value="2" {{$applicant->status == '2' ? 'selected' : ''}}>Selecionado</option>
                                          <option value="3" {{$applicant->status == '3' ? 'selected' : ''}}>Rejeitado</option>
                                        </select>
                                      </form>
                                </td>
                                <td>
        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="exampleModalLongTitle">{{ __('Exibir informações do candidato') }}</h5>
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">{{ __('Fechar') }}</button>
            </div>
          </div>
        </div>
      </div>
