		...
		<div class="row">
			<div class="col-lg-12">
				<div class="page-title-item text-center">
					<h2 class="title">{{ __('Vagas') }}</h2>
					
						<ul class="breadcrumb-nav">
							<li class=""><a href="{{ route('front.index') }}">{{ __('Home') }} </a></li>
							<li class="active" aria-current="page">{{ __('Vagas') }}</li>
						</ul>
			...
			<div class="col-lg-8">
				@if (count($jobs) == 0)
						<div class="col-md-12">
							...
							<h3 class="text-center">{{__('NENHUM VAGA ENCONTRADO')}}</h3>
							</div>
						</div>
				@else
				@foreach ($jobs as $job)
					<div class="single-job">
						...
						<p><span><i class="far fa-calendar-alt"></i>Prazo final :</span> {{ $job->deadline }}</p>
						<p><span><i class="fas fa-money-bill-wave"></i>Salário :</span> {{ $job->salary }}</p>
						...
						<p><span><i class="fas fa-gift"></i>Vagas :</span> {{ $job->vacancy }}</p>
						<a href="{{ route('front.careerdetails', $job->slug) }}">{{ __('Saiba Mais') }}<i class="fal fa-long-arrow-right"></i></a>
					</div>
					@endforeach
